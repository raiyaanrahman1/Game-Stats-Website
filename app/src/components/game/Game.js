...
class Game extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      coverArt:
        "https://upload.wikimedia.org/wikipedia/commons/8/83/Steam_icon_logo.svg", // sample image
  ...
  onClickLike = () => {
    console.log("like");
    likeGame({ user: this.props.user, gameId: this.state.gameID });
  ...
  ShowReviewToggle = () => {
    this.setState({ showReview: 1 - this.state.showReview });
  };
  showTHWToggle = () => {
    this.setState({ showTHW: 1 - this.state.showTHW });
  };
    ...
    return (
      <div className="game-main">
        ...
        <div className="game-buttons">
          {" "}
          {/* TODO: only shown if user is logged in */}
          <button className="game-button" onClick={this.ShowReviewToggle}>
            {" "}
            <span title="Write a Review">✎ Write a Review</span>
          </button>
          <button className="game-button" onClick={this.showTHWToggle}>
            <span title="Test my Hardware">☑ Test My Hardware</span>
          </button>
          <button
          ...
          >
            <span title="Add to Favourites">✰ Add to Favorites</span>
          </button>
        </div>
        ...
        <AddReview
          ...
          close={this.ShowReviewToggle}
        />

        <TestHardware
          showTHW={this.state.showTHW}
          userID={this.state.userID}
          close={this.showTHWToggle}
        />
      </div>
    );
  }
